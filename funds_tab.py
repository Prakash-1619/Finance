...
PROFILE_DOMAINS = {
    "Universal Finance": ["Loans", "EMI", "Car", "Cows", "Sheep", "Agri Land", "Home", "Personal", "Friends lending", "Miscellaneous"],
    "Personal Finance": ["Home", "Personal", "Friends lending", "Miscellaneous", "EMI"],
    "Farm & Livestock": ["Cows", "Sheep", "Agri Land", "Car", "Loans"],
    "Credit & Loans": ["Loans", "EMI"],
    "Compact Overview": ["Loans", "EMI", "Car", "Home", "Personal"]
}

DEFAULT_DOMAINS = PROFILE_DOMAINS["Universal Finance"]

def get_profile_domains(profile):
    return PROFILE_DOMAINS.get(profile, DEFAULT_DOMAINS).copy()


def get_active_domains(profile, preferred_domains, available_domains):
    if preferred_domains:
        selected = [domain for domain in preferred_domains if domain in available_domains]
        return selected or available_domains
    return available_domains


def get_available_domains(df_raw):
    if df_raw.empty:
        return []
    return [domain for domain in DEFAULT_DOMAINS if domain in df_raw['Domain'].unique()]


def get_compact_message(profile):
    if profile == "Compact Overview":
        return "Compact dashboard enabled: summary cards and essential sections only."
    return ""


def render_compact_summary(df_raw):
    if df_raw.empty:
        return
    st.markdown("### 🔎 Compact Overview")
    total_income = df_raw[df_raw['Transaction Type'].isin(['Income', 'Received'])]['Amount'].sum()
    total_expenditure = df_raw[df_raw['Transaction Type'].isin(['Expenditure', 'Paid'])]['Amount'].sum()
    net_balance = total_income - total_expenditure
    c1, c2, c3 = st.columns(3)
    c1.metric("Income", f"₹{total_income:,.2f}")
    c2.metric("Expense", f"₹{total_expenditure:,.2f}")
    c3.metric("Net Balance", f"₹{net_balance:,.2f}")


PROFILE_DESCRIPTIONS = {
    "Universal Finance": "Full dashboard with all major finance domains for broad money management.",
    "Personal Finance": "Focused on home, personal spending, EMI, and cash flow for household budgeting.",
    "Farm & Livestock": "Track agricultural income, cattle, sheep, and land operations in one view.",
    "Credit & Loans": "Manage loans, EMIs, and debt flow with a clean credit-centric dashboard.",
    "Compact Overview": "A concise summary for fast decisions and essential finance metrics only."
}

def render_profile_banner(profile):
    st.markdown(f"### {profile}")
    st.write(PROFILE_DESCRIPTIONS.get(profile, "Choose a profile that best matches your financial workflow."))
...
def render_flexible_plots(df, key_prefix):
    ...
    if selected_plots := st.multiselect(
...
def render_funds_tab(data=None, profile="Universal Finance", preferred_domains=None, compact=False):
    df_raw = data if data is not None else load_data()
    if df_raw.empty:
        st.info("No data found.")
        return

    df_raw['Date'] = pd.to_datetime(df_raw['Date']).dt.date
    available_domains = get_available_domains(df_raw)
    active_domains = get_active_domains(profile, preferred_domains, available_domains or get_profile_domains(profile))

    render_profile_banner(profile)
    ...
    if compact or profile == "Compact Overview":
        render_compact_summary(df)
        st.divider()

    tab_labels = ["📊 Overview", "🟩 Income", "🟥 Expenditure"]
    tab_labels += [f"💠 {domain}" for domain in active_domains]
    ...
    with tabs[0]:
        ...
        st.divider()
        render_delete_interface(filt_g, "global_overview")
    ...
    # 4. DOMAIN DASHBOARDS
    if active_domains:
        for i, d_name in enumerate(active_domains):
            render_domain_dashboard(d_name, tabs[i+3], df_raw, start_date, end_date)
    else:
        st.info("No active profile domains selected. Update the layout profile or preferred domains to show domain dashboards.")
